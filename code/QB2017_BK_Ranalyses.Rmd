...
# finding matrix elucidan distance among all individuals #
distance <- as.matrix(dist(BCI.Q.df, upper = TRUE))
# define intermediate df #
closest.neighbors <- NA
# Define number of individual/rows in distance matrix #
nrows <- nrow(distance)
for(i in 1:nrows) {
  ind.distance <- distance[which(rownames(distance)==i),which(colnames(distance)!=i)]
  ordered.neighbors <- order(ind.distance)[1:4]
  #neighbors <- names(ind.distance[ordered.neighbors])
  #closest.neighbors <- rbind(closest.neighbors, neighbors)
}
...
for(i in 1:nquad) {
  BCI.Q.dat <- subset(BCI, Quadrat == i , select = c(Latin, Quadrat, PX, PY))
  ilist <- BCI.Q.dat[sample(nrow(BCI.Q.dat), size = 5, replace = FALSE), ]

...
for(i in 3) {
  ...
  exp.neg <- BCIcooc$negative
  exp.pos <- BCIcooc$positive
  exp.rand <- BCIcooc$random
  df <- rbind(exp.neg, exp.pos, exp.rand)
  exp.df <- cbind(exp.df, df)
...
#map of four most abundant trees
...
Results from the study suggest that tree community composition on Barro Colorado Island in Panama is randomly distributed.  However, these results do not necessarily support the assumptions of the Unified Theory of Biodiversity and more research is needed. 
